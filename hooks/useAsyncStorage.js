...
export const AsyncStorageProvider = ({ children }) => {
  ...
  const init_user_settings = async () => {
    ...
    const init_settings = {
      ...
      battery_optimization_alert_send: false,
    };
  ...
  const deleteReceiver = async (receiverData) => {
    try {
      const jsonValue = JSON.stringify(
        Object.fromEntries(
          Object.entries(receiversList).filter(
            (receiver) => receiver[1] !== receiverData
          )
        )
      );
  ...
  const deleteStoredNotification = async (billId) => {
    try {
      const jsonValue = JSON.stringify(
        Object.fromEntries(
          Object.entries(storedNotifications).filter(
            ([id, notifications]) => id != billId
          )
        )
      );
  ...
  return (
    <AsyncStorageContext.Provider value={memoedValue}>
      {children}
