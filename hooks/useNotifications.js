...
export const NotificationsProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (
      ...
      const linkNotificationToBillDetailScreen = async () => {
        try {
          return await AsyncStorage.getItem("notifications").then(
            (storedNotifications) => {
              const [billId] = Object.keys(
                Object.fromEntries(
                  Object.entries(JSON.parse(storedNotifications)).filter(
                    ([id, notifications]) =>
                      notifications.find(
                        (notificationId) =>
                          notificationId ===
  ...
  async function scheduleNotifications(billId, dateExpiry, title) {
    const settings = await AsyncStorage.getItem("settings").then((result) =>
      JSON.parse(result)
    );
    const {
      isEnabled,
    ...
    const amountOfNotifications = Math.ceil(
      daysBeforeFirstNotification / repeatNotification
    );
      ...
      if (trigger1 > dateNow) {
        await Notifications.scheduleNotificationAsync({
          content: {
            title: title,
            body: `Je hebt nog ${
              daysBeforeFirstNotification - i * repeatNotification
            }${""} ${
              daysBeforeFirstNotification - i * repeatNotification === 1
                ? "dag"
                : "dagen"
            } om deze rekening te betalen!`,
            priority: Notifications.AndroidImportance.MAX,
          },
          trigger: {
            channelId: "default",
  ...
  async function getScheduledNotifications() {
    await Notifications.getAllScheduledNotificationsAsync().then((result) =>
      console.log(result)
    );
  }
